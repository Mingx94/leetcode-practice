...
var maxIncreaseKeepingSkyline = function(grid) {
  let verticalSkyline = grid.map((r, index) =>
    grid.reduce((max, row) => Math.max(max, row[index]), 0)
  );
  let horizontalSkyline = grid.map(row => Math.max(...row));

  return grid.reduce((total, row, i) => {
    total += row.reduce((rowIncrease, value, j) => {
      rowIncrease += Math.min(horizontalSkyline[i], verticalSkyline[j]) - value;
      return rowIncrease;
    }, 0);
    return total;
  }, 0);
